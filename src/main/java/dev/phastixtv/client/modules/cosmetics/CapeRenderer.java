    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, AbstractClientPlayerEntity abstractClientPlayerEntity, float f, float g, float h, float j, float k, float l) {
        if (!(MinecraftClient.getInstance().currentScreen instanceof TitleScreen) /*&& Mizu.moduleManager.getModule(Cape.class).isEnabled()*/ && !abstractClientPlayerEntity.isInvisible() && abstractClientPlayerEntity.isPartVisible(PlayerModelPart.CAPE) && abstractClientPlayerEntity.getName().equals(MinecraftClient.getInstance().player.getName())) {
            ...
            if (itemStack.getItem() != Items.ELYTRA) {
                ...
                VertexConsumer vertexConsumer = ItemRenderer.getArmorGlintConsumer(vertexConsumerProvider, RenderLayer.getArmorCutoutNoCull(new Identifier("mizuclient:textures/cosmetics/capes/poison/cape.png")), false, false);
