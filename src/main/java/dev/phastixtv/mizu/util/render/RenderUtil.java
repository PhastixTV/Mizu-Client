...
public class RenderUtil {
    private static final MinecraftClient mc = Mizu.INSTANCE.getMc();
    ...
    public static void fillRoundBottomRect(DrawContext context, int x, int y, int width, int height, int radius, int color) {
        fillRect(context, x, y - 10, width, height, color);
    ...
    public static void fillRoundRect(DrawContext context, int x, int y, int width, int height, int radius, int color) {
        ...
        buf.begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
        buf.vertex(mat, x + width / 2F, y + height / 2F, 0).color(color).next();
        ...
        for (int corner = 0; corner < 4; corner++) {
            ...
            for (int i = cornerStart; i <= cornerEnd; i += 10) {
                ...
                float rx = corners[corner][0] + (float)(Math.cos(angle) * radius);
                float ry = corners[corner][1] + (float)(Math.sin(angle) * radius);
